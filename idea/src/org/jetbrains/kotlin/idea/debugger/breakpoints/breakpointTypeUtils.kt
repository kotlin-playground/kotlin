...
fun computeVariants(
    ...
    lambdas.forEach {
        result.add(kotlinBreakpointType.ExactKotlinJavaBreakpointVariant(
                XSourcePositionImpl.createByElement(it.bodyExpression), it, ordinal++))
    }
...
fun getLambdasAtLineIfAny(file: KtFile, line: Int): List<KtFunction> {
    ...
    return allInlineFunctionCalls.filter { it.getLineNumber() == line && it.getLineNumber(false) == line }
}
